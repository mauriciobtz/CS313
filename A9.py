#  File: recursion2.py 100/100

#  Description: excercises for practicing recursion
...
def groupSum(start, nums, target):
    if (start >= len(nums)):  # checks if we've gotten to the end of the list
        return target == 0  # check if target was reached
    # else, subtract number from target
    else:
...
def groupSum6(start, nums, target):
    ...
    if start >= len(nums):
        return target == 0
    # checks if current list item is a 6, ensures it is chosen
    elif nums[start] == 6:
...
def groupNoAdj(start, nums, target):
    ...
    if start >= len(nums):
        return target == 0
    # the start + 2 makes sure adjacent numbers are not selected
    else:
...
def groupSum5(start, nums, target):
    ...
    # if the number is a multiple of 5 and it's not the last item, check if the
    # next item is a 1, and skip the 1 if it exists
    elif nums[start] % 5 == 0 and start < (len(nums) - 1) and nums[start + 1] == 1:
        return groupSum5(start + 2, nums, target - nums[start])

    # checks for multiples of 5 if they are the last number in the list
    elif nums[start] % 5 == 0:
...
def groupSumClump(start, nums, target):
    if start >= len(nums):
        ...
        counter = 0  # this will keep track of the amount of occurrences
        for i in range(start, len(nums)):
            if nums[i] == nums[start]:
                counter += 1  # increase counter if the next value is the same
            else:
                break  # then the next value is different, just break from the loop
        return groupSumClump(start + counter, nums, target) or groupSumClump(start + counter, nums, target - (
                    counter * nums[
                start]))  # you either take the lump or you don't so then you chance the start the same by the counter and if you take the clump you decrease the target by the sum of the clump
...
# (No loops needed.)
def splitArray(nums):
    return splitArrayHelper(0, 0, 0, nums)  # if index reaches, it's the end of nums


def splitArrayHelper(left_sum, right_sum, idx, nums):
    if (idx >= len(nums)):
        return left_sum == right_sum  # base case

    else:
        return (splitArrayHelper(left_sum + nums[idx], right_sum, idx + 1, nums) or splitArrayHelper(left_sum,
                                                                                                     right_sum + nums[
                                                                                                         idx], idx + 1,
                                                                                                     nums))  # this is the recursive case that compares the two baskets
...
def splitOdd10Helper(ten_sum, odd_sum, idx, nums):
    # base case
    if idx >= len(nums):
        return ten_sum % 10 == 0 and odd_sum % 2 != 0

    # selects number if it is a multiple of 10
    elif nums[idx] % 10 == 0:
        return splitOdd10Helper(ten_sum + nums[idx], odd_sum, idx + 1, nums)

    else:
        return splitOdd10Helper(ten_sum, odd_sum + nums[idx], idx + 1, nums)
...
def split53Helper(group5, group3, idx, nums):
    if idx >= len(nums):
        return group5 == group3
    elif nums[idx] % 5 == 0:  # checks if number is a multiple of 5

        return split53Helper(group5 + nums[idx], group3, idx + 1, nums)
    elif nums[idx] % 3 == 0:  # checks if the number is a multiple of 3

        return split53Helper(group5, group3 + nums[idx], idx + 1, nums)
    else:
        return split53Helper(group5 + nums[idx], group3, idx + 1, nums) or split53Helper(group5, group3 + nums[idx],
                                                                                         idx + 1, nums)
